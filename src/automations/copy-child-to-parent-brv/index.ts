...
export default async (req: NowRequest, res: NowResponse): Promise<void> => {
  return hasRole(['lambda', 'nidavellir', 'manticore'])(req, res, async (err: unknown) => {
    try {
      ...
      const parentBaseRun = await ultron.baseRun.findById(stepRun.baseRunId)

      const children = await ultron.baseRun.findMany({ where: { parentId: parentBaseRun.id } })
      const lastChild = children.slice(-1)[0]
      const child = await ultron.baseRun.findById(lastChild.id as string)

      const mapped = variables.map((v) => {
        const source = find({ baseVariableId: v.source }, child.baseRunVariables)
        const target = find({ baseVariableId: v.target }, parentBaseRun.baseRunVariables)
        if (!source)
      ...
      const updateVariables = mapped.map((row) => ({
        baseRunId: parentBaseRun.id as string,
